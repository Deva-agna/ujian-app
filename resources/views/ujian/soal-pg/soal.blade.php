...
<div class="card p-1">
    <div class="d-flex justify-content-between">
        <form action="{{route('soal.pg.pilih', $ujian->slug)}}">
            <div class="input-group input-group-merge ">
                ...
                <input type="text" name="cari" class="form-control form-control-sm" placeholder="Deskripsi. . ." value="{{request('cari')}}" aria-describedby="basic-addon-search2">
            </div>
        </form>
        <a href="{{ route('soal.pg.list', $ujian->slug) }}" class="btn btn-sm btn-secondary">
            <span>Kembali</span>
        </a>
    </div>
</div>

@forelse($soal_s->load('detailSoal') as $soal)
...
<div class="card">
    ...
    <div class="card-body">
        ...
        <div class="d-flex justify-content-between">
            <p>Jawaban :
                @if($soal->detailSoal[0]->kunci_jawaban)
                A
                @elseif($soal->detailSoal[1]->kunci_jawaban)
                B
                @elseif($soal->detailSoal[2]->kunci_jawaban)
                C
                @else
                D
                @endif
